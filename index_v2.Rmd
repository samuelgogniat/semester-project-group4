...
For preprocessing, analyses and visualising our datas we used R (v. 4.2.3; R Core Team, 2023) as well as the package ggplot2 (v. 3.4.2; Wickham, 2016).
...
On our own training dataset, we reached a overall accuracy of 91.9%. It has to mentioned though, that the accuracy reached depended strongly on the transport mode. For walks there were the most misclassifications i.e. walk to car (n=5), walk to bus (n=2) and walk to bike (n=7). For a total overview of misclassifications see figure 6.
...
In the predictive validation with the second dataset, overall accuracy was 60.9% and hence significantly lower. Looking at the confusion matrix in figure 7, it can be seen, that the most problematic allocations were between tram and bus which means that our method often classified bus as tram drives (n=50). The same happend quite often for cars which tended to be classified as tram (n=21), too. For bus drives our method often classified car (n=26). On the other hand many walks were treated as bus drives (n=17).
...
-   Wickham, H. (2016). ggplot2: Elegant Graphics for Data Analysis. Springer-Verlag New York. 216 pp. [https://ggplot2.tidyverse.org](http://127.0.0.1:43521/#0 "https://ggplot2.tidyverse.org")
